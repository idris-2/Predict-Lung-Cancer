...
df_transformed = pd.DataFrame({
    ...
    "fatigue": (df["ENERGY_LEVEL"] < 57).astype(int),
    "severe_fatigue": (df["ENERGY_LEVEL"] < 47).astype(int),
    ...
    "environmental_risk": ((df["EXPOSURE_TO_POLLUTION"] == 1) | (df["SMOKING_FAMILY_HISTORY"] == 1)).astype(int),
